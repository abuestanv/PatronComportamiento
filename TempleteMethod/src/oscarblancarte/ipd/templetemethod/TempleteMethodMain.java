...
import oscarblancarte.ipd.templetemethod.impl.EmpresaXYZFileProcess;
...
public class TempleteMethodMain extends TimerTask {
    ...
    private static final String[] PATHS = 
        new String[]{"C:/files/drugstore", "C:/files/grocery", "C:/files/empresaXYZ"};
    ...
    public void run() {
        ...
        if(!f.exists()){ throw new RuntimeException("El path '"+PATHS[0]+"' no existe"); }
        ...
        // Grocery Files
        f = new File(PATHS[1]);
        if(!f.exists()){ throw new RuntimeException("El path '"+PATHS[1]+"' no existe"); }
        ...
        // Empresa XYZ Files
        f = new File(PATHS[2]);
        if(!f.exists()){ throw new RuntimeException("El path '"+PATHS[2]+"' no existe"); }
